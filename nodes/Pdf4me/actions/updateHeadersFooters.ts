...
export async function execute(this: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
	try {
		...
		if (!hasContent) {
			throw new NodeOperationError(this.getNode(), 'At least one header or footer content must be provided', { itemIndex: index });
		}
		...
		throw new NodeOperationError(this.getNode(), `Update Headers and Footers failed: ${errorMessage}`, {
			itemIndex: index,
		});
	}
}
