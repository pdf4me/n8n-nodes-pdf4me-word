...
export async function execute(this: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
	try {
		...
		if (secureOnOpen && (!secureOnOpenPassword || secureOnOpenPassword.trim() === '')) {
			throw new NodeOperationError(this.getNode(), 'Open password is required when Secure On Open is enabled', { itemIndex: index });
		}
		...
		throw new NodeOperationError(this.getNode(), `Secure Word document failed: ${errorMessage}`, {
			itemIndex: index,
		});
	}
}
